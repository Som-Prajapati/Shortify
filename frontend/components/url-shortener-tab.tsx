...
export default function URLShortenerTab({
  ...
  const handleCreateShortener = async () => {
    ...
    try {
      ...
      const newUrl = `${selectedDomain}/${shortId}`;
      setShortenedUrl(newUrl);
      setOriginalUrl("");
      setCustomShortId("");
      setAvailable(null);

      // Show success toast and auto-dismiss after 4 s
      setSuccessToast(newUrl);
  ...
  const handleToastCopy = () => {
    ...
    navigator.clipboard.writeText(successToast);
    setToastCopied(true);
  ...
  return (
    <>
      ...
      .toast-slide-up {
        animation: slideUpFade 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
      }
    `}</style>

      {successToast && (
        <div
          className="toast-slide-up fixed bottom-8 left-1/2 z-50 flex items-center gap-3 px-5 py-4 rounded-2xl shadow-2xl border border-green-500/30 bg-card/95 backdrop-blur-md min-w-[320px] max-w-[90vw]"
          style={{ transform: "translateX(-50%)" }}
        >
          {/* Green glow ring */}
          <div className="flex-shrink-0 w-9 h-9 rounded-full bg-green-500/15 flex items-center justify-center ring-2 ring-green-500/30">
            <CheckCircle2 className="w-5 h-5 text-green-400" />
          </div>

          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold text-green-400 mb-0.5">
              Short URL created! See in History
            </p>
          ...
          <button
            ...
            className="flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-gray-200 hover:bg-gray-300 text-black border border-black transition-all duration-200"
          ...
          <button
            ...
            className="flex-shrink-0 text-foreground/30 hover:text-foreground/70 transition-colors duration-200 text-lg leading-none"
            title="Dismiss"
          >
            ×
          </button>
